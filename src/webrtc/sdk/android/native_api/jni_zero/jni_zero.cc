...
#include "sdk/android/native_api/jni_zero/jni_zero.h"

#include <sys/prctl.h>

#include "sdk/android/native_api/jni_zero/generate_jni/JniInit_jni.h"
#include "sdk/android/native_api/jni_zero/jni_methods.h"
#include "sdk/android/native_api/jni_zero/jni_zero_internal.h"
#include "sdk/android/native_api/jni_zero/logging.h"
...
jclass GetClassInternal(JNIEnv* env, const char* class_name,
  ...
  if (g_class_resolver != nullptr) {
    ...
    clazz = jni_help::FindClass(env, class_name);
  }
...
jclass LazyGetClassInternal(JNIEnv* env, const char* class_name,
                            const char* split_name,
...
void DisableJvmForTesting() { g_jvm = nullptr; }
...
jmethodID MethodID::Get(JNIEnv* env, jclass clazz, const char* method_name,
...
template <MethodID::Type type>
jmethodID MethodID::LazyGet(JNIEnv* env, jclass clazz, const char* method_name,
                            const char* jni_signature,
...
namespace internal {
jclass LazyGetClass(JNIEnv* env, const char* class_name, const char* split_name,
                    std::atomic<jclass>* atomic_class_id) {
...
jclass LazyGetClass(JNIEnv* env, const char* class_name,
                    std::atomic<jclass>* atomic_class_id) {
  jclass ret = atomic_class_id->load(std::memory_order_acquire);
  if (ret == nullptr) {
